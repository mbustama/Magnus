# cProfile of the PREM chord scan, order 4 GL, refinement off, both kernels on.
...
d = int(sys.argv[1]); n = int(sys.argv[2]); reps = int(sys.argv[3])
prem_call(d, n)   # warm numba
prem_call(d, n)
pr = cProfile.Profile()
pr.enable()
for _ in range(reps):
