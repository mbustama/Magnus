# End-to-end: PREM chord scan with the framing + GL4 fusions patched in.
# Expect bit-identical probabilities and a pure speed change.
...
def expm_fused(Om, warn_wide=False, A_is_const=False, expm_backend=None):
    ...
    if Om.dtype == np.complex128 and Om.ndim >= 3 and Om.flags.c_contiguous:
        ...
        if dev <= 1.e-12*scale:
            K = 1j*Om
            if (backend != 'eigh' and ek.HAVE_NUMBA and ek.supports_dim(d)):
...
for d in (2, 3, 4, 5):
    ...
    print("d=%d  totals(ms) old %6.2f/%6.2f/%6.2f new %6.2f/%6.2f/%6.2f  marginal us/slab/E %6.3f -> %6.3f (%4.2fx)  |dP| %.1e"
          % (d, *[1e3*res[(n,'old')][0] for n in NS], *[1e3*res[(n,'new')][0] for n in NS],
             1e6*mo, 1e6*mn, mo/mn, dP))
