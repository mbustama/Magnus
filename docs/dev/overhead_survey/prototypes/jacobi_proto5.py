...
def _jacobi_expm_warm_mgs2(K, out, lam):
    nB, d, _ = K.shape
    ...
    for b in range(nB):
        ...
        for _sweep in range(30):
            ...
            if off2 <= thr2:
                break
        # save unsorted V as the next warm start, re-orthonormalized by
        # modified Gram-Schmidt so non-unitarity cannot compound along the chain
        for i in range(d):
            for j in range(d):
                V0[i, j] = V[i, j]
        for j in range(d):
            for m in range(j):
                acc = 0.0 + 0.0j
                for k in range(d):
                    acc += np.conj(V0[k, m])*V0[k, j]
                for k in range(d):
                    V0[k, j] -= acc*V0[k, m]
            nrm = 0.0
            for k in range(d):
                nrm += V0[k, j].real**2 + V0[k, j].imag**2
            nrm = 1.0/np.sqrt(nrm)
            for k in range(d):
                V0[k, j] *= nrm
        have_warm = True
        for m in range(d):
            f[m] = complex(np.cos(A[m, m].real), -np.sin(A[m, m].real))
        for i in range(d):
            for j in range(d):
                acc = 0.0 + 0.0j
                for m in range(d):
                    acc += V0[i, m]*f[m]*np.conj(V0[j, m])
                out[b, i, j] = acc
        for i in range(d):
            lam[b, i] = A[i, i].real
        for i in range(1, d):
