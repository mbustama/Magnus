...
failure; when the author says "audit," all of it applies, to captions exactly as
to body text.
...
**10. Condense** wherever it costs no clarity, correctness, or impact.
...
**12. No very long paragraphs.** Break at an appropriate place. **Measure from the
source file, where a paragraph is blank line to blank line** — not from extracted
PDF text, where a displayed equation splits one paragraph into several blocks and
the length check never sees it.
...
**16. Style, read off the author's own published papers.** Measure, don't assume:
  ...
  b. *Voice*: first person through the body, not only the introduction — roughly
     ...
     densest in methods and results. Plain active verbs.
  ...
  d. *Numbers*: two or three significant figures, units always explicit, hedging
     quantified rather than linguistic ("3 sigma", not "seems likely").
  e. *Captions*: two to five sentences, self-contained, panels and line styles
     ...
     only lists what is plotted has failed. The headline must name what the
     plotted quantity *is*, in the reader's vocabulary, and claim only what the
     axes show. No fitted slopes or mantissas anywhere; scale statements instead.
  ...
  i. *Habits*: em dashes for asides; italics for variables and defined terms,
     ...
     achievement and draw implications; they do not re-report results.
...
than apology. A scope statement is precision, never a concession.

**18. Every figure and table gets a full introduction in the text**, in a paragraph
that *opens* on it ("Figure 7.2 shows …"). The introducing paragraph says what the
...
headline that editorializes ("A front is a smooth function or a discontinuity
depending on...") must be replaced by one that names the plotted quantity.

**21. The "and" tic, in both of its forms.** Never weld a statement to a
clarification with ", and" -- "the two live at different scales, and the lower one
...
clause to the other, or use a semicolon. This is stricter than criterion 15 and
supersedes it: classify every ", and" by hand, keep only the serial comma and genuine
compound predicates sharing one subject.

**22. Succinct first, but never at the expense of clarity or correctness.** Criterion 10
condenses only where it costs nothing --- this one says what "costs nothing" means.
Where the choice is between a shorter sentence and one a reader can follow without
decoding, write for the reader; length is secondary. A passage that is correct and
clear at 150 words beats one that is cryptic at 60, and a term used is a term
defined, however much space the gloss takes. This governs criteria 10, 12 and 19:
condense and merge only when the result still reads to someone meeting the material
for the first time, not to someone who already knows what it refers to. Both halves bind. Always strive for succinct text: cut the word, the clause and the
...
No dangling words on a line of their own. **Rerun after every fix until clean** —
each reflow moves the line breaks, and fixing one dangle routinely creates another.
...
## D. Standing rules that override local judgment
