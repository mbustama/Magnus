...
\begin{figure*}[t!]
 ...
 \includegraphics[width=\textwidth]{strategies.pdf}
  \caption{\label{fig:strategies}The seven engines that can answer a request, in the order in which they are tried. Each row draws what its engine does to the trajectory: nothing propagated at all for the closed-form average, which reads $\langle P \rangle$ from the eigenbasis of $\mathbb{H}$; adiabatic transport with an exact Magnus patch across the resonance; the fast vacuum phase removed analytically before a single Magnus pass; one exponential for the whole path where $\mathbb{H}$ does not vary; one evaluation of the profile shared across many energies; one pass returning many baselines; and the general ladder, refining until two successive stages agree. Shading is the matter density. An engine declines a request whose conditions it does not meet, and the request passes to the next engine, so the last is reached whenever no other applies; \tabl{engines} gives the conditions.}
\end{figure*}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{table*}[t!]
 ...
 \hline
 Closed-form average & {\tt 'average'} & An averaged probability is requested and $\mathbb{H}$ is position-independent \\
 ...
 Constant Hamiltonian & {\tt 'constant'} & $V_{\rm CC}$ does not vary along the trajectory; any flavor count, a single point or a scan \\
...
A request with one shared value reaches one of the batched engines of Sec.~\ref{sec:engines}, which reuse work across the array. The \textit{energy-batched scan} evaluates the profile once and carries the energy as a batch dimension through every slab; the \textit{cumulative scan} traverses the profile once and records the running product at every requested baseline. Both run in a single process. The gain is that the profile is not traversed once per point, at no cost in accuracy: batched answers are bit-identical to the point-by-point ones (Sec.~\ref{sec:validation}).
%%%%
