...
%%%%
{\em Solution method:}\\
The evolution operator is propagated with the Magnus expansion. Truncated time-ordered integrals of the Hamiltonian are evaluated by Gauss--Legendre quadrature over a chain of position slabs and exponentiated; every truncation remains in the Lie algebra, so the operator is unitary by construction. Slab widths are chosen adaptively from a requested tolerance, and slab edges are placed on the discontinuities of a layered profile. Where the accumulated phase is extreme, an adiabatic transport is composed with exact local Magnus patches across the non-adiabatic windows. The phase-averaged probability is available without resolving the oscillation: each interference term is weighted by the spread of its phase across the energy resolution. Where every phase has run through many cycles, this is the decohered limit. \\ \\
\smallskip
%%%%
...
\begin{equation}
 ...
 {(\lambda_k - \lambda_j)^2} \;.
\end{equation}
The second quantity measures how non-adiabatic the pair of levels $(j, k)$ is: small where the levels are far apart compared with how fast $\mathbb{H}$ changes, large where they approach. (For two flavors, $\gamma_{jk} = 1/(2\gamma_{\rm LZ})$, where $\gamma_{\rm LZ} = \lvert \lambda_k - \lambda_j \rvert / (2\lvert d\theta_m/dl \rvert)$, with $\theta_m$ the mixing angle in matter, is the adiabaticity parameter of the Landau-Zener literature.) Both expressions need only the derivative of $\mathbb{H}$, not the derivative of an eigenvector, which would depend on the above arbitrary phase. 
...
\end{enumerate}
These are properties of the limit. Where part of an interference term survives (see below), none of the three holds in general. The probability then depends on the baseline and is not symmetric under the exchange of $\alpha$ and $\beta$. In vacuum, CP violation again distinguishes neutrinos from antineutrinos.
...
\end{equation}
with $\phi_{ij} = (\lambda_i - \lambda_j)L$ and $\phi'_{ij} = d\phi_{ij}/d\ln E$. The damping factor is the average of $e^{-i\phi}$ over a Gaussian spread of width $\sigma$ in $\ln E$, the form familiar from wave packets and finite resolution\ \cite{Kiers:1995zj, Blennow:2005yk, Ohlsson:2000mj}. In vacuum, $\phi' = -\phi$. At $\sigma = 10\%$, a phase of $2\pi$ keeps 82\% of its interference; a phase of 20~rad keeps 14\%; a phase of 40~rad keeps $3\times10^{-4}$. Equation (\ref{equ:phase_average}) is therefore \equ{averaged} wherever every phase runs through many cycles, the oscillation probability itself where none does, and a smooth interpolation in between. We obtain the slopes from the Hellmann-Feynman identity of \equ{hf}, $d\lambda_i/d\ln E = \langle v_i|\, d\mathbb{H}/d\ln E\, |v_i\rangle$, with $d\mathbb{H}/d\ln E$ from a central difference, so they cost two more evaluations of $\mathbb{H}$ and no diagonalization. The spread is set with {\tt average\_spread} and is $10\%$ by default.

Only the phases are averaged: $\mathbb{V}$ stays at the central energy. That is not the same as averaging the probability over an energy band. Because $\mathbb{H}$ depends on the energy, a band average also averages over different Hamiltonians. In vacuum, where the eigenvectors do not depend on energy, the phase average and the band average differ only through the second-order dependence of the phase on the energy offset. In matter, their difference is set by how fast the eigenvectors change with energy. On the BS2005-AGS,OP solar curve from 0.1 to 20~MeV, a $10\%$ band average moves 26 of 40 energies by more than $10^{-4}$. Equation (\ref{equ:phase_average}) moves none of them, since that curve carries no interference. Wherever \equ{phase_average} agrees with the limit to $10^{-4}$, \magnus\ returns the limit, computed as before.
...
Equation (\ref{equ:averaged_varying}) composes the crossings as probabilities and reads the result out incoherently. It therefore keeps no phase at all---neither between two crossings nor between the last crossing and the end of the path, however short that stretch is. The phase average carries its definition onto the path instead. An energy offset $u = \delta \ln E$ moves each instantaneous eigenvalue by $u\, d\lambda_i/d\ln E$ and leaves the eigenvectors unchanged. We average the evolution under $\mathbb{H} + u\, \mathbb{D}_{\rm diag}$ over a Gaussian in $u$ of width $\sigma$, with $\mathbb{D}_{\rm diag}$ the part of $d\mathbb{H}/d\ln E$ diagonal in the instantaneous eigenbasis. \magnus\ computes it without sampling energies along the path: each crossing is a Magnus patch evaluated at a few Gauss-Hermite nodes in $u$, each adiabatic stretch between crossings a phase with an exact slope in $u$. The answer then does not depend on where the crossing windows are drawn. Where there is no crossing, it is \equ{averaged_varying}. Against a brute-force average of the same definition on five chords through the Sun, from 10~GeV to 10~TeV, it agrees to $4 \times 10^{-5}$; \equ{averaged_varying} is off by up to 0.14 on the same chords (Sec.~\ref{sec:solar_tomography}).
...
\item \emph{Phase average.} This engine answers every request made with {\tt average=True}, by one of three routes chosen by $\mathbb{H}$. When $\mathbb{H}$ does not depend on position, it returns \equ{phase_average}, the probability with each interference term damped by the spread of its phase across the energy resolution. That is the quantity a solar or an astrophysical experiment measures. Where every phase has decohered, it is the limit \equ{averaged}. The engine diagonalizes $\mathbb{H}$ once and propagates nothing. When $\mathbb{H}$ varies smoothly along the path, the engine transports along the instantaneous eigenstates, keeping each phase with its weight, with the Magnus patch of Sec.~\ref{sec:hybrid} across every crossing that is not adiabatic. Without such a crossing, the result is \equ{averaged_varying}. When the profile declares discontinuities, there is no instantaneous eigenbasis; the engine then propagates for real across a window in energy and averages over it, a different quantity (Sec.~\ref{sec:average_keyword}). See Sec.~\ref{sec:astro} for an example.
...
\begin{figure*}[t!]
 ...
 \caption{\label{fig:strategies}The seven \magnus\ engines that can answer a probability request, in the order in which they are tried. Each row draws what its engine does to the trajectory: for the phase average, $\langle P \rangle$ in closed form from the eigenbasis of $\mathbb{H}$ when one eigenbasis serves the whole path, otherwise transported along the instantaneous eigenstates with a Magnus patch across each crossing, each phase kept and weighted by its spread in energy; adiabatic transport with an exact Magnus patch across the resonance; the fast vacuum phase removed analytically before a single Magnus pass; one exponential for the whole path where $\mathbb{H}$ does not vary; one evaluation of the profile shared across multiple energies; one pass returning multiple baselines; and the general Magnus ladder, refining until two successive stages agree. Shading represents the matter density. An engine declines a request whose conditions it does not meet; in that case, the request passes to the next engine, so the last engine (general Magnus ladder) is reached whenever no other applies; \tabl{engines} gives the conditions.}
\end{figure*}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
