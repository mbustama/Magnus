...
The scenes themselves live in ``notebooks/27_magnus_animations.ipynb``, which
...
do with neutrinos.  This file is shared with the sibling project
NuOscProbExact, where an earlier version carried its own copy of the
scenes and that copy had already drifted --- it still explained that an
Earth curve of a hundred energies was a hundred calls, which had stopped
being true a release earlier.  The encoding is the same problem in both
repositories; the physics is not, and only the notebook owns it.
...
::
    ...
    # 1. Render the scenes: set RENDER = True in notebook 27.  The raw clips
    #    land in img/raw/, which is gitignored.
    # 2. Shrink each one into img/, which IS tracked -- this is the step that
    #    turns a 160 MB render into a file worth committing:
    python tools/make_demo_video.py --shrink img/raw/anim_cp.gif \
        --out img/anim_cp.gif --fps 12 --width 860 --colors 128

    # 3. Or join them into one reel, for showing rather than committing:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog='The scenes are rendered by notebooks/27_magnus_animations.ipynb; '
